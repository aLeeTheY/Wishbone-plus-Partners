...
| :----------------------------------: | :-------: | :---------------------: | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------: |
|   `-W, --workspace-version, --wv`    | `boolean` |         `false`         |                                                                         Displays current workspace semantic version                                                                          |
...
| `-I, --internationalization, --i18n` | `boolean` |         `false`         |                                                                    Activates localized multi-lingual document compilation                                                                    |
|       `--inline-sprite, --is`        | `boolean` |         `false`         |                                              Directly embeds the SVG vector asset grid inside the document (forced automatically via `--local`)                                              |
|         `--inline-css, --ic`         | `boolean` |         `false`         |                                                            Directly embeds production stylesheets inside the HTML layout payload                                                             |
|         `--inline-js, --ij`          | `boolean` |         `false`         |                                                           Directly embeds processed client scripts inside the HTML layout payload                                                            |
|      `-M, --mobile-first, --mf`      | `boolean` |         `false`         | Toggles the build layout pipeline. Sets PostCSS media query sorting to `mobile-first` and configures the HTML picture transformer to generate `min-width` source tags instead of `max-width` |
|  `-d, --domain, --site-url, --url`   | `string`  | `http://localhost:3000` |                                                               Maps target deployment domain root, e.g., `https://example.com`                                                                |
| `-F, --site-folder, --folder, --sf`  | `string`  |           `/`           |                                                  Folder name on the hosting server or GitHub repository name (e.g., `my-repo`, `www`, ...)                                                   |
| `-P, --prod-server, --server, --ps`  | `boolean` |         `false`         |                                                           Launches a local server tracking the compiled production build footprint                                                           |
| `-H, --secure, --use-https, --https` | `boolean` |         `false`         |                                                      Forces HTTPS protocol execution for both build paths and local development servers                                                      |
|    `-g, --gh-pages, --demo, --gh`    | `boolean` |         `false`         |                                                 Enable GitHub Pages build configuration (uses GitHub URLs, see `site.config.json` in `src/`)                                                 |
...
|         Option         |   Type    | Default Value |                                                                Description                                                                 |
| :--------------------: | :-------: | :-----------: | :----------------------------------------------------------------------------------------------------------------------------------------: |
| `--obfuscation, --obf` | `boolean` |    `false`    | Obfuscates structural CSS class selectors across HTML, CSS, and JS. Incompatible with `--inline-css`, `--inline-js`, and `--inline-sprite` |
