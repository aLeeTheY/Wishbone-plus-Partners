...
>
> Если конфигурацию необходимо переопределить «на лету» без правки файла, можно использовать CLI-флаги `--domain` (`-d`) и `--site-folder` (`-F`).

> [!NOTE]
> **Умная нормализация (защита от ошибок).**
>
> В сборщик встроена автоматическая обработка слэшей. Вы можете указывать их в любом формате (ставить в конце, начале или вовсе опускать) — Gulp-сборка самостоятельно нормализует строки, предотвращая поломку путей к ресурсам.
